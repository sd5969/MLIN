...
<?php
	...
	$trimmed = explode(" ",htmlspecialchars (trim($_GET['q']), ENT_QUOTES, 'UTF-8'));
	$size = sizeof($trimmed);
	$query = "SELECT *, DATE_FORMAT(dateTime, '%M %e, %Y, %l:%i%p') AS newdate FROM stories WHERE";
	for ($i = 0; $i < $size; $i++) {
		$query .= " (story LIKE '%" . $trimmed[$i] . "%') ";
		if($i<($size-1)){
			$query .= "AND ";
		}
	}
...
?>
<title>MLIN - My Life Is Nerdy | It's the Nerd Life - Search</title>
...
<!-- STORIES -->
<!-- STORIES -->
<p>Search "<?php echo htmlentities($_GET[q]); ?>" results:</p><br />
<?php
	if(mysql_num_rows($result8) == 0) echo "Sorry, 0 results were returned.  Try other search terms.<br /><br />";
...
?>
<a <?php if(1 == $page) echo "class='pageSelected boxSelected'"; else echo "class='page box'"; ?> href="/search/<?php echo $q; ?>/1">&nbsp;1&nbsp;</a>
<?php
	if($page > 6) echo "... ";
	$startcount = $page - 6;
...
?>
<a <?php if($i + 1 == $page) echo "class='pageSelected boxSelected'"; else echo "class='page box'"; ?> href="/search/<?php echo $q . "/" . ($i + 1); ?>">&nbsp;<?php echo $i + 1; ?>&nbsp;</a>&nbsp;
<?php
	}
	if($page != 1 && $page != $count2) {
?>
<a class='pageSelected boxSelected' href="/search/<?php echo $q . "/" . $page; ?>">&nbsp;<?php echo $page; ?>&nbsp;</a>&nbsp;
<?php
	}
	$endcount = $page + 5;
...
<?php
	...
	if($count2 != 1) {
?>
<a <?php if($count2 == $page) echo "class='pageSelected boxSelected'"; else echo "class='page box'"; ?> href="/search/<?php echo $q . "/" . $count2; ?>">&nbsp;<?php echo $count2; ?>&nbsp;</a>&nbsp;
<?php
	}
?>
